...
import type { GeminiResponse, GeminiStreamResponse, GeminiStreamEvent } from '../types/gemini';

/**
 * Validates that the sender is from a valid file:// URL
 ...
 */
ipcMain.handle('gemini:generate', async (event, prompt: string): Promise<GeminiResponse> => {
  try {
 ...
 */
ipcMain.handle('gemini:stream', async (event, prompt: string): Promise<GeminiStreamResponse> => {
  try {
    // Validate sender frame URL
    if (!validateSender(event)) {
      throw new Error('Invalid sender: only file:// protocol allowed');
    ...
    try {
      // Stream the response
      for await (const chunk of stream(sanitizedPrompt)) {
        event.sender.send('gemini:stream', {
          type: 'chunk',
          streamId,
          data: chunk,
 ...
 */
ipcMain.handle('gemini:generate-json', async (
  event, 
  prompt: string, 
  ...
  try {
    ...
    if (template && !JSON_TEMPLATES[template]) {
      throw new Error(`Invalid template: must be one of ${Object.keys(JSON_TEMPLATES).join(', ')}`);
    }
    ...
    const hasWarnings = (
      (template === 'summary' && response.title === 'Unable to generate title') ||
